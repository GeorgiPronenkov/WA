...
function paginateData(dataInput, currentPage, perPage) {

	 let curr_Page =  1;
	 let per_Page = 10;
	 let offset = (curr_Page - 1) * perPage;
	 let resultData = dataInput.map(x => x.title).slice(offset).slice(0, perPage);
	 let total_pages = Math.ceil(dataInput.length / per_Page);
	 return {
...
console.log(paginateData(dataInput));